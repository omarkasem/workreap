...
if (!class_exists('WorkreapProposalFunctions')) {
    class WorkreapProposalFunctions
    {
        ...
        public function workreap_submit_proposal(){
            ...
            $proposal_data  = !empty($_POST['data']) ? $_POST['data']: '';
...
if( !function_exists('workreapProjectDisputeComments') ){
    function workreapProjectDisputeComments($user_id=0,$request=array(),$type=''){
        ...
        if( !empty($action_type) && $action_type === 'reply' ){
            ...
            // if freelancer resolve dispute
            do_action('workreap_freelancer_resolve_project_dispute', $request);
    ...
    function workreapDeclineProposal($user_id=0, $proposal_id=0, $detail='', $type=''){
        ...
        if( !empty($user_id) && !empty($proposal_id)){
            if( empty($detail) ){
                ...
                $wr_post_data['post_status']    = 'declined';
						...
						<?php if(!empty($order_ids)){?>
							<ul class="wr-planslist">
								...
								foreach ($order_ids as $order_id) {
                                    ...
                                    $employer_side_process_fee = !empty($product_data['processing_fee']) ? $product_data['processing_fee'] : 0;
								    $get_total = $get_total - $employer_side_process_fee;
                                    ...
                                    <?php if(!empty($employer_side_process_fee) && $user_type != 'freelancers'){ ?>
                                    <li>
										<h6>
											<?php echo esc_html('Processing Fee', 'workreap-child'); ?>
											<span>(<?php workreap_price_format($employer_side_process_fee); ?>)</span>
										</h6>
									</li>
                                    <?php } ?>
								<?php } ?>
